		...
		<tbody>
			<c:forEach var="p" items="${cart.itemsCart}">
				<tr>
					<td>${p.id}</td>
					<td>${p.name}</td>
					<td><fmt:formatNumber value="${p.unitPrice}" type="currency" currencySymbol="₫" pattern="###,###.##"/>${" ₫"}
					</td>
					<td><f:formatNumber value="${p.discount}" type="percent" /></td>
					<td>${p.quantity}</td>
					<td>
						<fmt:formatNumber value="${p.unitPrice*p.quantity*(1-p.discount)}" type="currency" currencySymbol="₫" pattern="###,###.##"/>${" ₫"}
					</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
	<div class="panel-footer">Tổng giá : <fmt:formatNumber value="${cart.amountCart}" type="currency" currencySymbol="₫" pattern="###,###.##"/>${" ₫"}</div>
</div>
		...
		<div class="panel-body">
			...
			<div class="row">
				<div class="form-group col-sm-6">
					...
					<div class="form-control">
						<fmt:formatNumber value="${cart.amountCart}" type="currency" currencySymbol="₫" pattern="###,###.##"/>${" ₫"}
					</div>
				</div>
				<div class="form-group col-sm-6">
					<label>Ghi chú</label>
